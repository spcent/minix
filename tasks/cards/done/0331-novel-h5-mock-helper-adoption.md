...
- [x] Novel H5 mock adapter removes local response, path, and query coercion duplicates
- [x] mock route behavior remains unchanged
- [x] `pnpm verify` run, or skipped with reason if docs-only

## Completion Notes

- Replaced Novel H5 local mock response, path, query number, and query string helpers with core mock request helpers.
