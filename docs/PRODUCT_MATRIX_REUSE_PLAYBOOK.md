...
- Use `packages/core/src/store/snapshot.ts` when shared protocol or controller code needs immutable state snapshots. Prefer this small data-only helper over scattering raw clone calls in reusable factories, workspace/inbox controllers, and form-heavy controllers.
