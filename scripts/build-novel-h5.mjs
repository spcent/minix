...
function resolveNovelH5Paths(repoRoot = process.cwd()) {
  const appDir = path.join(repoRoot, "apps", "novel-h5");
...
function createNovelH5Html(template) {
...
async function writeNovelH5Html(distDir, templateFile) {
  const template = await readFile(templateFile, "utf8");
  await writeFile(path.join(distDir, "index.html"), createNovelH5Html(template), "utf8");
}
...
export async function buildNovelH5(repoRoot = process.cwd()) {
  const paths = resolveNovelH5Paths(repoRoot);
  ...
  await writeNovelH5Html(paths.distDir, paths.htmlTemplateFile);
  console.log(`novel-h5 build written to ${paths.distDir}`);
  return paths.distDir;
}

export async function watchNovelH5(repoRoot = process.cwd()) {
  const paths = resolveNovelH5Paths(repoRoot);
  await mkdir(paths.assetsDir, { recursive: true });
  await syncPublicFiles(paths.publicDir, paths.distDir);
  await writeNovelH5Html(paths.distDir, paths.htmlTemplateFile);
  ...
  console.log(`novel-h5 watch active in ${paths.appDir}`);
