...
function buildHostWechatIndexWxml(entry: HostWechatPageShellConfig): string {
  ...
  <view class="card landing-card landing-cta">
    ...
    <view class="subtitle">{{authStatus === 'reauth_required' ? 'Sign in again to continue. The original route id, params, and source are preserved.' : (authenticated ? (redirectTarget ? 'You are signed in. Continue back to the requested page, or choose another unlocked page from Home.' : 'You are signed in. Home stays in place and the rest of the product unlocks through the menu and actions below.') : 'Home explains the product and previews the lesson shape. The rest of the product unlocks after sign-in.')}}</view>
    <view class="status-text">{{loading ? 'Preparing your lesson...' : (authStatus === 'reauth_required' ? (redirectLabel ? 'Sign in again to continue to the protected page.' : 'Sign in again to continue.') : (authenticated ? (redirectTarget ? 'Signed in. Continue to the requested page.' : 'Signed in. Choose where to go next.') : 'Ready for today\\'s practice'))}}</view>
    ...
    <button wx:if="{{authenticated && redirectTarget}}" class="button" type="primary" bindtap="onTapContinueDestination">
      Continue to {{redirectLabel || 'Requested Page'}}
    </button>
